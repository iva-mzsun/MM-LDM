...
if __name__ == "__main__":
    ...
    if opt.suffix is None or opt.suffix=="":
        sampledir_video = os.path.join(logdir, f'samples-videos-{ckpt_name}-ucgs{opt.ucgs_video}-long')
        sampledir_audio = os.path.join(logdir, f'samples-audios-{ckpt_name}-ucgs{opt.ucgs_audio}-long')
        sampledir_mm = os.path.join(logdir, f'samples-mm-{ckpt_name}-ucvid{opt.ucgs_video}-ucaud{opt.ucgs_audio}-long')
    else:
        sampledir_video = os.path.join(logdir, f'samples-videos-{ckpt_name}-ucgs{opt.ucgs_video}-long-{opt.suffix}')
        sampledir_audio = os.path.join(logdir, f'samples-audios-{ckpt_name}-ucgs{opt.ucgs_audio}-long-{opt.suffix}')
        sampledir_mm = os.path.join(logdir, f'samples-mm-{ckpt_name}-ucvid{opt.ucgs_video}-ucaud{opt.ucgs_audio}-long-{opt.suffix}')
    ...
    print(f"- Saving generated long video samples to {sampledir_video}")
    print(f"- Saving generated long audio samples to {sampledir_audio}")
    print(f"- Saving generated long multimedia samples to {sampledir_mm}")
    batch_idx = 0
    for batch in tqdm(iter(dataloader)):
        start = time.time()
        sample_log = model.log_samples(batch, N=bs, n_frames=video_length, ar_times=3,
                                       verbose=verbose, sample=False, ddim_steps=ddim_step,
                                       ucgs_video=opt.ucgs_video, ucgs_audio=opt.ucgs_audio,
                                       audio_continuation=opt.audio_continuation,
                                       video_continuation=opt.video_continuation,
        ...
        video = sample_log[f"video_sample_ucgs{opt.ucgs_video}"] # b c t h w
        ...
        for b, name in enumerate(item_names):
            ...
            audios = []
            for audio_item in all_audios:
                audio = model.stable_ae.audio_reconstructer.reconstruct(audio_item[b], raw_audio_size[b])
                audios.append(audio)
