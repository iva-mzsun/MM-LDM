...
class VideoAudioDataset_wTXT_Long(Dataset):
    def __init__(self, ids_file,
        ...
        for cid in ids:
            ...
            if len(frames) >= self.nframe * 2:
                items.extend([(cid, i / video_fps, (i + self.nframe * 2) / video_fps)
                              for i in range(len(frames) - self.nframe * 2 + 1)])
        self.items = items
        ...
        if fix_prompt is None:
            raise NotImplementedError # TODO, load prompt from file
    ...
    def __getitem__(self, idx):
        cid, pre_stime, tar_etime = self.items[idx]
        pre_etime = (pre_stime + tar_etime) / 2
        tar_stime = (pre_stime + tar_etime) / 2

        prompt = self.fix_prompt
        pre_video_item = self.video_manager.obtain_video_sample(cid, pre_stime, pre_etime)
        pre_audio_item = self.audio_manager.obtain_img_from_wav(cid, pre_stime, pre_etime)
        tar_video_item = self.video_manager.obtain_video_sample(cid, tar_stime, tar_etime)
        tar_audio_item = self.audio_manager.obtain_img_from_wav(cid, tar_stime, tar_etime)

        return dict(id=cid,
                    txt=prompt,
                    duration=tar_etime - tar_stime,
                    audio_img=tar_audio_item['audio_img'],
                    raw_audioimg_size=tar_audio_item['raw_audioimg_size'],
                    key_frames=tar_video_item['key_frames'],
                    pre_frames=tar_video_item['pre_frames'],
                    tar_frames=tar_video_item['tar_frames'],
                    pre_audio_img=pre_audio_item['audio_img'],
                    pre_key_frames=pre_video_item['key_frames'],
                    pre_all_frames=pre_video_item['all_frames'],
                    tar_frame_indexes=tar_video_item['tar_frame_indexes'])
