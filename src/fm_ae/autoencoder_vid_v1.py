...
class AEUnetModel(UNetModel):
    def forward(self, x_video, x_audio,
        ...
        x = torch.cat([x_video], dim=0)
        timesteps = torch.cat([timesteps_video], dim=0)
        context = torch.cat([context_video], dim=0)
        ...
        for i, module in enumerate(self.input_blocks):
            ...
            feats = torch.cat([cond_feats_video[i]], dim=0)
            h = h + feats
            hs.append(h)
        # decode
        ...
        h = h.type(x.dtype)
        h = self.out(h)
        return dict(audio=x_audio, video=h)
