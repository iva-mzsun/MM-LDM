...
from src.fm_ae.utils.blocks import ResBlockwoEmb, TemporalAttentionBlock, \
    SpatialAttentionBlock, ResBlock2nwoEmb

from ipdb import set_trace as st

class AudioContentEnc(nn.Module):
    def __init__(
        self,
        image_size,
        in_channels,
        ...
        if self.learnable_content:
            ...
            self.audio_content = nn.Parameter(torch.randn(shape), requires_grad=True)
        # create input blocks
        ...
        for i, ch in enumerate(sdinput_block_chans):
            ...
            if tards == ds:
                layers = [ResBlockwoEmb(out_channels, dropout, ch)]
            elif ds < tards:
                while curds < tards:
                    n += 1
                    curds *= 2
                layers = [ResBlock2nwoEmb(out_channels, dropout, ch, down=True, sample_num=n)]
            else:
                while curds > tards:
                    n += 1
                    curds /= 2
                layers = [ResBlock2nwoEmb(out_channels, dropout, ch, up=True, sample_num=n)]
    ...
    def encode(self, x):
        ...
        if self.learnable_content:
            B, C, H, W = x.shape
            vc = repeat(self.audio_content, 'c h w -> b c h w', b=B)
            x = torch.cat([vc, x], dim=1)

        x = x.unsqueeze(2) # b c t h w
        h = x.type(self.dtype)
    ...
    def decode(self, latent):
        ...
        for module in self.output_blocks:
            cond_feat = module(latent)
            cond_feats.append(cond_feat)
        return cond_feats

    def forward(self, x, is_training=False, return_latent=False, **kwargs):
        '''
            x: [b, c, t, h, w]
            ind: [b, t]
        '''
