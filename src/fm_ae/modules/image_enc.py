...
class ImageContentEnc(nn.Module):
    def __init__(
        self,
        image_size,
        in_channels,
        ...
        if self.learnable_content:
            ...
            self.image_content = nn.Parameter(torch.randn(shape), requires_grad=True)
        # create input blocks
        ...
        for i, ch in enumerate(sdinput_block_chans):
            ...
            if tards == ds:
                layers = [ResBlockwoEmb(out_channels, index_embed_dim, dropout, ch)]
            elif ds < tards:
                while curds < tards:
                    n += 1
                    curds *= 2
    ...
    def encode(self, x):
        ...
        if self.learnable_content:
            B, C, H, W = x.shape
            vc = repeat(self.image_content, 'c h w -> b c t h w', b=B, t=1)
